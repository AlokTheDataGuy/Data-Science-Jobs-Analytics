...
import { fetchFilteredCompanies } from "../../api";
import {
  BarChart,
  Bar,
  XAxis,
...
export default function TopCompaniesChart({ role = "all", state = "all" }) {
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white px-4 py-3 rounded-lg shadow-xl border border-pink-200">
          <p className="font-semibold text-gray-800">
            {payload[0].payload.company}
          </p>
          <p className="text-pink-600 font-bold text-lg">
            {payload[0].value} openings
          </p>
        </div>
      );
    }
  ...
  return (
    <>
      ...
      <Modal
        ...
        title="Top Companies - Job Openings"
      >
        {ChartBody}
      </Modal>
    </>
